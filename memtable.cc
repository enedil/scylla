            ...
            if (!_delegate) {
                ...
                if (_delegate_range) {
                    ...
                    if (key_and_snp) {
                        ...
                        auto mpsr = make_partition_snapshot_flat_reader_from_snp_schema(_slice.options.contains(query::partition_slice::option::reversed), _permit, std::move(key_and_snp->first), std::move(cr), std::move(key_and_snp->second), digest_requested, region(), read_section(), mtbl(), streamed_mutation::forwarding::no, *mtbl());
...
private:
    void get_next_partition() {
        ...
        if (key_and_snp) {
            ...
            auto snp_schema = key_and_snp->second->schema();
            // FIXME: is it correct not to reverse in flush reader?
...
memtable::make_flat_reader(schema_ptr s,
    ...
    if (query::is_single_partition(range) && !fwd_mr) {
        ...
        flat_mutation_reader rd = make_partition_snapshot_flat_reader_from_snp_schema(is_reversed, std::move(permit), std::move(dk), std::move(cr), std::move(snp), digest_requested, *this, _read_section, shared_from_this(), fwd, *this);
        rd.upgrade_schema(s);
        return rd;
    } else {
